        ...
        {#if currPage === "one"}
            <div class="flex gap-20 items-center w-full mt-5">
                <div class="flex flex-col gap-2 w-full">
                    <label  for="expense_types" class="block font-serif text-sm">Expense Type</label>
                    <select id="expense_types" class="hover:cursor-pointer font-sans w-full h-10" bind:value={chosenExpenseType}>
                ...
                <div class="flex flex-col gap-2 w-full">
                    <label class="font-serif text-sm" for="expense_title">Title</label>
                    <input name="expense_title" class="w-full font-sans h-10 rounded border-primary-black" type="text" id="expense_title" bind:value={expenseTitle}>
                </div>
            </div>

            <div class="flex gap-20 items-center w-full mt-5">
                <div class="flex flex-col gap-2 w-full">
                    <label class="font-serif text-sm" for="cost">Cost</label>
                    <input name="cost" class="font-sans w-full h-10 rounded border-primary-black" type="text" id="cost" bind:value={expenseCostDisplay} on:input={updateExpenseCost}>
                </div>
                <div class="flex flex-col gap-2 w-full">
                    <label class="font-serif text-sm" for="join_date">Date</label>
                    <input name="join_date" class="font-sans w-full h-10 rounded border-primary-black" type="date" id="join_date" bind:value={date}>
                </div>
            </div>
            
            
            <div class="flex flex-col gap-2 mt-5">
                <label class="font-serif text-sm" for="note">Note</label>
                <textarea name="note" class="font-sans rounded border-primary-black" bind:value={note} id="note"></textarea>
            </div>
            ...
            <div class="flex flex-col gap-2 mt-5">
                {#if expenseDocumentChosen === false}
                    <label class="font-serif text-sm" for="contract">Expense Document (Not required*)</label>
                ...
                {:else}
                    <div class="flex flex-col gap-2">
                        <p class="text-lg font-serif">File</p>
            ...
            <div class="mt-5">
                ...
                {:else}
                    <Button 
                        height=10 width=36 label="Add Expense" fontSize="sm" padding="8px"
